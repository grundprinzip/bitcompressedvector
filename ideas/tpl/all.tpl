...
public:
    ...
    template<int offset>
    inline static int overlap_value(const __m128i& data, int* output);
...
template<>
template<>
inline int BitCompression<<%bits%>>::overlap_value<<%offset%>>(const __m128i& data, int* output)
{
...
{
    int *ctr = out;
    register __m128i qw_block = *block;
    <%#extracts%>
    ...
    __m128i tmp = _mm_alignr_epi8(*(block + 1), qw_block,  <%base_shift%>);
